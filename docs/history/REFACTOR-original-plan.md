...
> **📦 ARCHIVED — historical design record.** This is the *original* phased
> build plan (Phases 0–6) written before the refactor was implemented. The
> modular architecture it specifies (`core/`, `qc/`, `ancestry/`, `gwas/`,
> `cli/`) is now built, so this plan is superseded. **The living source of truth
> is `REFACTOR.md` at the repo root** (formerly `REFACTOR_HARDENING.md`) — see it
> for current state, the round-by-round change log, and remaining work. Its
> "Conventions to Preserve" section (below) is fully mirrored in `CLAUDE.md`.
> Kept only as a reference for the original design rationale.
