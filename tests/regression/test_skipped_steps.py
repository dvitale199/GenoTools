...
def _pfile_with_phenos(src: Path, dst: Path, pheno_of) -> Path:
    """Copy src's pfiles to dst, rewriting PHENO1 as pheno_of(row_index)."""
    ...
    pheno_col = header.index("PHENO1")
    rows = [lines[0]]
    for i, line in enumerate(lines[1:]):
        fields = line.split()
        fields[pheno_col] = str(pheno_of(i))
...
def test_case_control_without_both_phenotypes_is_skipped(
    ...
    """
    cases_only = _pfile_with_phenos(
        test_geno_path, tmp_path / "cases_only", lambda i: 2
    )
...
def test_case_control_runs_when_both_phenotypes_present(
    ...
    """
    both = _pfile_with_phenos(
        test_geno_path, tmp_path / "both_phenos", lambda i: 1 + (i % 2)
    )
    out = tmp_path / "runcc"
